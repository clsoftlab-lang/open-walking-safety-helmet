# Documentation · 문서

> Part of the Open Walking Safety Helmet · Licensed CC BY 4.0 · © 2026 CLSOFTLAB, Dr. Lee Il-guk

*Until the day blind people everywhere can walk like anyone else.*
전 세계 시각장애인이 일반인처럼 다닐 수 있는 그날까지.

The system design specification is the single source of truth. When a guide and the spec disagree, the spec
wins. Please open an issue.

설계서가 유일한 기준 문서입니다. 가이드와 설계서가 다르면 설계서를 따르고, 이슈로 알려 주십시오.

## Where to start · 어디서 시작할까요

- **I want to use the helmet** (users, families, O&M instructors):
  [User guide](en/05-user-guide.md) → [Safety](en/06-safety.md)
- **헬멧을 사용하려고 합니다** (사용자, 가족, 보행지도사):
  [사용자 가이드](ko/05-사용자가이드.md) → [안전 수칙](ko/06-안전수칙.md)
- **I want to build one:**
  [Bill of materials](en/01-bill-of-materials.md) → [Buying guide](en/10-buying-guide.md) → [3D printing](en/03-3d-printing-guide.md) →
  [Wiring](en/02-wiring-guide.md) → [Assembly](en/04-assembly-guide.md) → [Test plan](en/07-test-plan.md)
- **직접 만들려고 합니다:**
  [부품 목록](ko/01-부품목록.md) → [구매 가이드](ko/10-구매가이드.md) → [3D 프린팅](ko/03-3D프린팅가이드.md) →
  [배선](ko/02-배선가이드.md) → [조립](ko/04-조립가이드.md) → [시험 계획](ko/07-시험계획.md)
- **I want to contribute:** [CONTRIBUTING.md](../CONTRIBUTING.md) · [Roadmap](en/08-patent-and-roadmap.md) ·
  [Translation guide](en/09-translation-guide.md)

## All documents · 전체 문서

| # | English | 한국어 | For |
|---|---|---|---|
| 00 | [System design specification](en/00-system-design-spec.md) | [시스템 설계서](ko/00-시스템설계서.md) | Everyone building or changing anything |
| 01 | [Bill of materials](en/01-bill-of-materials.md) · [bom.csv](bom.csv) | [부품 목록](ko/01-부품목록.md) | Builders |
| 02 | [Wiring guide](en/02-wiring-guide.md) | [배선 가이드](ko/02-배선가이드.md) | Builders |
| 03 | [3D printing guide](en/03-3d-printing-guide.md) | [3D 프린팅 가이드](ko/03-3D프린팅가이드.md) | Builders |
| 04 | [Assembly guide](en/04-assembly-guide.md) | [조립 가이드](ko/04-조립가이드.md) | Builders |
| 05 | [User guide](en/05-user-guide.md) | [사용자 가이드](ko/05-사용자가이드.md) | Users, families, O&M instructors |
| 06 | [Safety](en/06-safety.md) | [안전 수칙](ko/06-안전수칙.md) | Everyone |
| 07 | [Test plan](en/07-test-plan.md) | [시험 계획](ko/07-시험계획.md) | Builders, testers, O&M instructors |
| 08 | [Patent mapping and roadmap](en/08-patent-and-roadmap.md) | [특허 대응표와 로드맵](ko/08-특허와로드맵.md) | Contributors |
| 09 | [Translation guide](en/09-translation-guide.md) | [번역 가이드](ko/09-번역가이드.md) | Translators, reviewers |
| 10 | [Buying guide (verified links)](en/10-buying-guide.md) | [구매 가이드 (실제 구매 링크)](ko/10-구매가이드.md) | Builders |

## Same list, without a table · 표 없이 같은 목록

English:

- [00 System design specification](en/00-system-design-spec.md)
- [01 Bill of materials](en/01-bill-of-materials.md), with [bom.csv](bom.csv)
- [02 Wiring guide](en/02-wiring-guide.md)
- [03 3D printing guide](en/03-3d-printing-guide.md)
- [04 Assembly guide](en/04-assembly-guide.md)
- [05 User guide](en/05-user-guide.md)
- [06 Safety](en/06-safety.md)
- [07 Test plan](en/07-test-plan.md)
- [08 Patent mapping and roadmap](en/08-patent-and-roadmap.md)
- [09 Translation guide](en/09-translation-guide.md)
- [10 Buying guide](en/10-buying-guide.md)

한국어:

- [00 시스템 설계서](ko/00-시스템설계서.md)
- [01 부품 목록](ko/01-부품목록.md)
- [02 배선 가이드](ko/02-배선가이드.md)
- [03 3D 프린팅 가이드](ko/03-3D프린팅가이드.md)
- [04 조립 가이드](ko/04-조립가이드.md)
- [05 사용자 가이드](ko/05-사용자가이드.md)
- [06 안전 수칙](ko/06-안전수칙.md)
- [07 시험 계획](ko/07-시험계획.md)
- [08 특허 대응표와 로드맵](ko/08-특허와로드맵.md)
- [09 번역 가이드](ko/09-번역가이드.md)
- [10 구매 가이드](ko/10-구매가이드.md)

## Project files · 프로젝트 파일

- [Project README](../README.md) · [한국어 README](../README.ko.md)
- [Patent pledge · 특허 무상 실시 선언](../PATENT_PLEDGE.md)
- [Contributing](../CONTRIBUTING.md) · [Code of conduct](../CODE_OF_CONDUCT.md) ·
  [Governance](../GOVERNANCE.md) · [Security](../SECURITY.md)
- [Licence notice](../NOTICE)

## Other languages · 다른 언어

Documentation in other languages is welcome under `docs/<lang>/`. See the [translation guide](en/09-translation-guide.md).
